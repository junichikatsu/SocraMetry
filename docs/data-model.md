...
```jsonc
{
  ...
  "completedAt": null,
  "lastSeenAt": 1786000300000,     // 最後の操作。中断の検出だけに使う（表示しない）
  "awayMs": 600000,                // 中断していた時間の合計。時間条件から差し引く
  ...
  "conclusions": [
    {
      "bodyHash": "9f2c…",        // 冪等判定（api-spec.md §4）
      "body": "コネクションのクローズ漏れ…",  // マスキング済み。中断からの復旧に要る
      "verdict": null,
      "feedback": "もう少し具体的に…",
      "at": 1786000400000
    }
...
`lastSeenAt` と `awayMs` は**時間条件のためだけ**にある。

Gate A → B の自動遷移も Gate C の解放も「詰まった人を助けるための安全弁」で
あって評価のための計測ではない（socratic-engine.md §7 / P2）。
助ける対象は画面の前で詰まっている人なので、席を外していた時間を数えない。

**`startedAt` は書き換えない。** 履歴の並びと表示に使う値なので、
「いつ始めたか」は事実として動かさず、「どれだけ向き合っていたか」を別に持つ。

`SESSION_ABANDON_AFTER_MS`（既定 24 時間）を超えて操作がなければ
`status: "abandoned"` にして再開させない。**勝手に `completed` にはしない。**
未解決のまま中断した、という事実を残す。

> **`reports.totalElapsedMs` の意味が v0.1 の途中で変わった。**
> 中断の記録を持つ前のレポートはカレンダー時間、以降は `awayMs` を引いた実作業時間。
> セッション数が少なくそもそも横比較に耐えない段階のため（evaluation-model.md §3.5）
> 移行はしていないが、混在していることをここに記録しておく。

### 原因宣言の本文（#27）

`conclusions` は冪等判定用のハッシュに加えて**本文も保存する。**
ハッシュだけだと、中断から再開したときに自分が何を書いたか復元できない。

保存するのは `errorText` と同じくマスキング後なので、**扱うデータの種類は増えない**
（security.md §3 の判断基準「その入力が LLM に届くか」を通過済みのもの）。
...
| **2 つの書き込みが `sessions` を上書きし合う** | **逐次操作を前提にしない**（下記）。診断は書き込み直前に読み直し、診断に属するフィールドだけを移す。`turns` への追記は「読んだ配列に append して put」とし、`seq` の重複は上記の冪等判定で吸収する |
| 状態フラグが並行更新で巻き戻る | `diagnosisStatus` は**写しであって真実の源ではない**。診断が使えるかは `session_secrets` にアイテムがあるかで判断し、読み出せたらフラグを自己修復する |
| 診断がいつまでも完了しない | 一定時間（既定 45 秒）を過ぎたら `focusHints` なしの汎用モードで出題を続ける（FR-15 / NFR-O4）。**待ち続けて詰ませない** |

> **完全な排他は諦める設計にしている。** 楽観ロック用のバージョン列を持たせても、
> compare-and-swap がストア側にない以上は原子的に守れない。
> 代わりに、競合が起きても**壊れず、同じ結果になる**ように操作を冪等に設計する。

### ★ 「単一ユーザーの逐次操作」を前提にしてはいけない

本書の旧版は「セッションは単一ユーザーの逐次操作が前提であり、実害のある競合は
ほぼ起きない」としていた。**これは誤りだった**（v0.1 の実装中に実環境で発現した）。

**[ADR-006](architecture.md#adr-006-sse-を廃止し診断の先行実行で体感速度を確保する) が
意図的に並行リクエストを作っている。** 診断は 20 秒級で、その間に利用者は
Gate B へ進み、設問に回答している。1 セッション = 1 アイテム（D1）なので
書き込みは常に**アイテム全体の置き換え**であり、後に書いた方が相手の変更を消す。

| 書き込みの順序 | 失われるもの | 症状 |
|---|---|---|
| 利用者 → 診断 | `gate` / `currentStage` / 出題済みのターン | 進んだはずの設問が消える |
| 診断 → 利用者 | `diagnosisStatus`（`ready` → `pending` に巻き戻る） | 以降の設問が永久に `202` を返し、クライアントが再試行の末に諦める |

**同一利用者でも、設計が並行実行を要求している以上「逐次」ではない。**
上表の 3 つの対策はいずれもこの前提の訂正から来ている。

> **教訓**: 「ほぼ起きない」と書いた競合は、その競合を**設計が能動的に作っている**
> かどうかを確かめてから書く。ここでは ADR-006 がまさにそれを作っていた。
...
**対象は利用者が入力するすべてのテキスト**（エラーテキスト / コード断片 / 直前の変更 /
...
| **ローカル絶対パス** | `/Users/...` / `/home/...` / `C:\Users\...` で始まるパスの、**ファイル名より前をすべて** | `<path>/<file>` |
| JWT | `eyJ` で始まる 3 セグメント | `[REDACTED_JWT]` |
| **除外語リスト** | 環境変数 `MASK_WORDS` に登録した社名・製品名の完全一致 | `[REDACTED_NAME]` |

実装は `packages/core/src/masking.ts`。**LLM を使わない決定的な正規表現**で行う
（LLM に送る前に処理する必要があるため、LLM は使えない）。
**同じ純関数をフロントにもバンドルし、送信前のプレビュー表示に使う。**
検出ルールと設計判断の詳細は [security.md §3](security.md#3-秘匿情報のマスキングr1-への対策)。
