...
| 11 | **`HTTP_TRIGGER_PATH` がトリガーのパスと一致しているか**（不一致だと全リクエストが 404 になる / ADR-009） |
| 12 | `timeout` が API 仕様の想定レイテンシ（最大 20 秒）より長いか |
| 13 | `LOG_LEVEL` が `INFO` 以下か（`DEBUG` 以上は入力内容がログに出うる） |

1〜4 は CI で自動チェックする。5〜13 は初回セットアップ時と設定変更時に手動で確認する。
