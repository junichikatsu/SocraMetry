...
| レイヤ | 採用技術 | 選定理由 |
|---|---|---|
| 言語 | TypeScript（バックエンド）/ 素の JavaScript（フロント） | BE は型で契約を守る。FE は素の JS のまま JSDoc で型検査する（ADR-013） |
| **フロントエンド** | **フレームワークなし。HTML + CSS + 素の JavaScript + esbuild** | 依存を増やさない。バンドラだけ入れるのはマスキングをサーバと共有するため（ADR-013） |
...
### ADR-013: フロントエンドをフレームワークなしで書く（バンドラのみ入れる）

**採用**: v0.1 ／ **改訂**: v0.1（#27。「ビルド工程を持たない」を撤回した）

**決定**: React / Next.js を使わず、**HTML + CSS + 素の JavaScript** で書く。
**バンドラ（esbuild）だけを入れる。**

```
apps/web/
├── src/                入力。素の JavaScript（JSDoc + checkJs）
│   ├── main.js         3 ゲートの進行と配線
│   ├── api.js          API 呼び出し規約（202 の再送を含む）
│   ├── thread.js       対話スレッドの描画
│   ├── report.js       結果カード（算出根拠を含む）
│   ├── dashboard.js    個人ダッシュボードと履歴
│   ├── radar.js        5 軸レーダー（SVG 手書き。ライブラリなし）
│   └── mask-preview.js 送信前マスキングプレビュー
└── public/             出力。apps/function が配信する（ADR-012）
    ├── index.html
    ├── styles.css
    └── app.js          ★ 生成物。git 管理しない
```

**フレームワークを入れない理由**（初版から変わらない）:

| # | 理由 |
|---|---|
| 1 | ルーティングも、複雑な状態管理も要らない。フレームワークが解く問題が存在しない |
| 2 | **ADR-012 と噛み合う。** 静的ファイルをそのまま返せる形になる |
| 3 | 依存パッケージが増えないことは、そのまま攻撃面が増えないことでもある |

**バンドラを入れた理由**（改訂の中身）:

初版は「ビルド工程が消える」ことを利点として挙げていた。**これを撤回する。**

[security.md §3](security.md#3-秘匿情報のマスキングr1-への対策) は
**送信前のマスキングプレビュー**を v0.1 の必須としている。これを実装するには
サーバと同じ純関数（`packages/core/src/masking.ts`）をブラウザに届ける必要があり、
そのためにはバンドルが要る。

> **プレビュー用に正規表現を書き写す案は採らなかった。**
> 2 つの実装が乖離した瞬間、画面では消えているのにサーバには生で届く、という
> **無いより悪い表示**になる。マスキングは「消えて見えること」自体が機能なので、
> 表示と実体がずれる余地を残せない。

したがって**入れたのはバンドラだけ**である。トランスパイルもフレームワークも入れていない。
`src/` に置くのは素の JavaScript で、ブラウザがそのまま解釈できる構文しか使わない。
esbuild がやるのは import の解決と 1 ファイルへの結合だけで、**出力は読める JS のまま**
（`minify: false`）にしてある。

副次的に、フロントから `@socrametry/shared` の公開型を JSDoc で参照できるようになった。
`tsc --checkJs` が API 契約とのずれを拾う（初版の「フロントは型を参照しない」は無効）。
...
`apps/web/src/**` に対して lint で禁止済み。

**v0.2 以降**: 組織ダッシュボード（複数画面・グラフ・一覧）が入る段階で、
フレームワーク導入を再検討する。**v0.1 の画面数には過剰である**というだけで、
将来にわたって不要という判断ではない。
...
│
├── apps/
│   ├── web/                      # ★ フロントエンド（バンドラのみ / ADR-013）
│   │   ├── build.mjs                       # esbuild。src/ → public/app.js
│   │   ├── src/                            # 入力。素の JS（JSDoc + checkJs）
│   │   │   ├── main.js                     #   3 ゲートの進行と配線
│   │   │   ├── api.js                      #   API 呼び出し規約（202 の再送）
│   │   │   ├── thread.js                   #   対話スレッドの描画
