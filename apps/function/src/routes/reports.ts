...
/**
 * 1 セッションの実測コスト（F11）。**api-spec.md には無い追加のエンドポイント。**
 * これが無いと実測コスト表を埋めるのに実行環境のログを人が読むしかなく、
 * デモ台本 #6「コストログを見せる」も画面から出せない。
 */
reportRoutes.get('/v1/sessions/:id/cost', async (c) =>
  c.json(await getSessionCost(c.get('auth'), sessionIdOf(c))),
)
